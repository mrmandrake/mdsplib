...
namespace mdsplib.DSP
{
    ...
    public static class Convert
    {
        ...
        {
            public static double[] RealPart(System.Numerics.Complex[] vecIn)
            {
                return (from c in vecIn select c.Real).ToArray();
            }

            public static double[] ImagPart(System.Numerics.Complex[] vecIn)
            {
            ...
            public static double[] ToMagnitudeSquared(System.Numerics.Complex[] rawFFT)
            {
            ...
            public static double[] ToMagnitude(System.Numerics.Complex[] rawFFT)
            {
                UInt32 np = (UInt32)rawFFT.Length;
                double[] mag = new double[np];
                for (UInt32 i = 0; i < np; i++)
            ...
            public static double[] ToMagnitudeDBV(System.Numerics.Complex[] rawFFT)
            {
                UInt32 np = (UInt32)rawFFT.Length;
                double[] mag = new double[np];
                for (UInt32 i = 0; i < np; i++)
            ...
            public static double[] ToPhaseDegrees(System.Numerics.Complex[] rawFFT)
            {
            ...
            /// <param name="rawFFT"> Complex[] input array"></param>
            /// <returns>double[] Phase (Degrees)</returns>
            public static double[] ToPhaseRadians(System.Numerics.Complex[] rawFFT)
            {
                UInt32 np = (UInt32)rawFFT.Length;
                double[] phase = new double[np];
                for (UInt32 i = 0; i < np; i++)
